...
/**
 * Process a single LinkedIn task with an assigned Unipile account.
 */
  ...
  try {
    ...
    if (jobConfig?.jobDescription) {
      ...
      try {
        ...
        console.warn(`${tag} [2/3] Analysis FAILED after ${Date.now() - analysisStart}ms: ${analysisErr.message}`);
      }
    } else {
...
async function logAccountPoolState(label: string) {
  ...
  console.log(`[AccountPool][${label}] ACTIVE=${active} BUSY=${busy} COOLDOWN=${cooldown} | atMinuteLimit=${atMinuteLimit} atDailyLimit=${atDailyLimit}`);
}
...
async function processLoop(): Promise<void> {
  ...
  while (Date.now() - startTime < MAX_LOOP_MS) {
    cycleCount++;
    console.log(`[Process] ── Cycle #${cycleCount} (elapsed ${Date.now() - startTime}ms / ${MAX_LOOP_MS}ms) ──`);
  ...
  if (remaining > 0) {
    console.log(`[Process] Time limit reached after ${cycleCount} cycle(s) — ${remaining} tasks still PENDING. Self-chaining...`);
    triggerProcessing().catch((err: any) => {
